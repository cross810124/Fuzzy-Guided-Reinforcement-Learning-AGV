...
class NavigationEnv(gym.Env):
    ...
    def _compute_reward(self, action: np.ndarray) -> Tuple[float, Dict]:
        ...
        linear_vel = obs[3]   # 線速度
        angular_vel = obs[4]  # 角速度 (自轉速度)
        goal_angle = obs[8]   # 目標夾角
        cos_theta = np.cos(goal_angle)

        # 1. 改進的 Progress Reward：
        # 只有當「方向對了 (cos_theta > 0)」且「真的有靠近」，才給較大獎勵
        dist_diff = self.prev_goal_dist - goal_dist
        if dist_diff > 0:
            # 方向越正，前進的獎勵加倍 (最高 2x)
            r_progress = dist_diff * 50.0 * (1.0 + max(0, cos_theta))
        else:
            # 遠離目標時，扣分要重，逼它轉回來
            r_progress = dist_diff * 60.0 

        # 2. 嚴厲打擊「自轉」：
        # 懲罰過大的角速度，尤其是當它沒有在前進的時候
        r_spin_penalty = -0.01 * abs(angular_vel)
        
        # 3. Heading 獎勵改為「門檻制」：
        # 不要一直給，只有在對準時給一點點「對準獎勵」
        r_heading = 0.02 * cos_theta if cos_theta > 0.9 else 0.0
        ...
        # 5. 靜止懲罰：
        # 如果完全不動，給予微小懲罰
        r_static = -0.02 if abs(linear_vel) < 0.02 else 0.0

        total = r_progress + r_spin_penalty + r_heading + r_time + r_static

        info = {
            "r_progress": r_progress,
            "r_spin": r_spin_penalty,
            "goal_dist": goal_dist,
        }
        return total, info
